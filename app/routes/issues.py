...
def get_issue(issue_id : str):
    """Get a specific issue by using id"""
    issues = load_data()
    for issue in issues:
...
def put_issue(issue_id : str, payload : IssueUpdate):
    """Update an issue by using its id"""
    issues = load_data()
...
def delete_issue(issue_id : str):
    ...
    for index, issue in enumerate(issues):
        if issue["id"] == issue_id:
            ...
            save_data(issues)
            return
    raise HTTPException(status_code=status.HTTP_204_NO_CONTENT , detail="Issue not found")
